/*
File: FnBrowseForFile.js
Method of opening a Windows load/save dialog (credit to Matti for the original).
*/

function BrowseForFile(Title, Dir, Save, Ext, File, HWndP, Filter)
{
	Title = NAME + " | " + Title;
	if (Filter === undefined)
	{
		Filter = "Messenger Plus! Live interfaces (*.xml)|*.xml|Interface Writer backup files (*.tmp)|*.tmp|Plain text files (*.txt)|*.txt|Any types of files (*.*)|*.*||";
	}
